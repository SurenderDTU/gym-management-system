...
router.get('/', async (req, res) => {
    try {
        const plans = await pool.query('SELECT * FROM plans WHERE gym_id = 1 ORDER BY price ASC');
        res.json(plans.rows);
    } catch (err) {
...
// CREATE A NEW PLAN (Now with Discount Fields)
    ...
    try {
        ...
        const newPlan = await pool.query(
            ...
            [
                ...
                is_popular || false, 
                description || '',
                parseInt(discount_percent) || 0,       // New Field: Discount %
                discount_valid_until || null           // New Field: Valid Until Date
            ]
        );
...
// UPDATE PLAN (Now with Discount Fields)
    ...
    try {
        ...
        const updatePlan = await pool.query(
            ...
            [
                ...
                is_popular || false, 
                description || '',
                parseInt(discount_percent) || 0,       // New Field
                discount_valid_until || null,          // New Field
                id
            ]
        );
...
router.get('/:id/analytics', async (req, res) => {
    ...
    try {
        // 1. Get Basic Plan Details
        const planResult = await pool.query("SELECT name, price FROM plans WHERE id = $1", [id]);
